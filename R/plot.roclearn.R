if (getRversion() >= "2.15.1") {
  utils::globalVariables(c("X1", "X2"))
}
#' Plot Decision Boundary for Linear AUC Maximization Models
#'
#' Visualize the fitted decision function of a linear AUC maximization model
#' using two selected features. A scatter plot of the observed data is overlaid
#' with the decision boundary where \eqn{f(x) = 0}.
#'
#' @param x A fitted model object of class \code{"roclearn"} (linear).
...
plot.roclearn <- function(x, ...) {
  ...
  resolve_one <- function(feat) {
    ...
    if (!is.null(levels) && !is.null(levels[[raw_name]])) {
      ...
      stop(sprintf(
        "Feature '%s' expanded into multiple dummy columns. Specify 'levels=list(%s=\"<Level>\")'. Candidates: %s",
        raw_name, raw_name, paste(cand, collapse = ", ")
      ), call. = FALSE)
    }
  }
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::labs(
      title = "ROC-LEARN Decision Function (f(x)=0 boundary)",
